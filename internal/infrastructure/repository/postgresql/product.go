...
func (p productRepo) GetAllProducts(ctx context.Context) ([]entity.Product, error) {
	ctx, span := otlp.Start(ctx, productServiceName, productSpanRepoPrefix+"GetAll")
	defer span.End()
	...
	if err != nil {
		return nil, fmt.Errorf("ProductRepo - GetAllProducts - r.Builder: %w", err)
	}

	rows, err := p.db.Pool.Query(ctx, sql, args...)
	if err != nil {
		return nil, fmt.Errorf("ProductRepo - GetAllProducts - r.Pool.Query: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&product.ID, &product.Name, &product.Description, &product.CategoryID, &product.UnitPrice, &createdAtTime, &updatedAtTime)
		if err != nil {
			return nil, fmt.Errorf("ProductRepo - GetAllProducts - rows.Scan: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("ProductRepo - GetAllProducts - rows.Err: %w", err)
	}
